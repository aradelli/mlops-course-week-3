...
class TransformerFeaturizer(BaseEstimator, TransformerMixin):
    def __init__(self, dim: int, sentence_transformer_model: SentenceTransformer):
        self.dim = dim
...
class NewsCategoryClassifier:
    def __init__(self, config: dict) -> None:
        ...
        featurizer = TransformerFeaturizer(
            config['featurizer']['sentence_transformer_embedding_dim'],
    ...
    def predict_proba(self, model_input: dict) -> dict:
        ...
        """
        preds = self.pipeline.predict_proba(np.array([model_input]))

        return {l: p for l, p in zip(self.pipeline.classes_, preds[0])}
...
def startup_event():
    ...
    """
    global classifier
    classifier = NewsCategoryClassifier(GLOBAL_CONFIG['model'])
...
def predict(request: PredictRequest):
    ...
    log = {
            ...
            'latency': f"{latency.total_seconds() *1000} ms"
    }
    logs.write(f"{json.dumps(log)}\n")
    return response
